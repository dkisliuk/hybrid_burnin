#!/usr/bin/python
# ITSDAQTestClasses.py -- classes relevant for representing ITSDAQ results summary files
# Created: 2018/07/29, Updated: 2018/08/01
# Written by Matthew Basso

import os
#from databaseUtilities import commands as dbCommands
...
class ResultsSummaryFile(object):
            ...
            if line[0] == '%':
                ...
                if '%NewTest' in line:
                    ...
                    tests[test_number]['JSON'].update({
                        ...
                        'problem':          True if lines[i+8][17:-1] == 'YES' else (False if lines[i+8][17:-1] == 'NO' else None)
                    })
    ...
    def printSummary(self):
        ...
        header = ['Index', 'Serial number', 'Test type', 'Date', 'Time', 'Run number', 'Passed', 'Problem']
        print(row_format.format(*header))
        print(150 * '-')
        for i, test in enumerate(self.tests):
            row = [str(i), test['JSON']['properties']['SERIAL_NUMBER'], test['JSON']['testType'], test['JSON']['date'], test['JSON']['properties']['TIME'],
                    test['JSON']['runNumber'], 'YES' if test['JSON']['passed'] == True else 'NO', 'YES' if test['JSON']['problem'] == True else 'NO']
        ...
        for test_number in args:
            try:
                ...
                if self.__checkSerialNumber(serial_number):
                    ...
                    component = dbCommands['getComponent'].run(component = serial_number)
                    self.tests[test_number]['JSON'].update({'code': component['code'], 'institution': component['institution']['code']})
                ...
                # Else -- if it is not a valid serial number
                    ...
                    component_list = dbCommands['listComponentsByProperty'].run(project = 'S', componentType = 'HYBRID', propertyFilter = property_filter)
                    ...
                    if len(component_list) == 1:
                        ...
                        raise GeneralError('Component not found.')
    ...
    def uploadTests(self, *args):
        if args == ():
            args = range(len(self.tests))
        for test_number in args:
            try:
                dbCommands['uploadTestRunResults'].run(self.tests[test_number]['JSON'])
            except IndexError:
                pass
            except TypeError:
                pass
...
class ResultsDirectory(object):
    ...
    def getResultsSummaryFilepaths(self):
        self.filepaths = sorted(list(filter(lambda filepath: filepath[-4:] == '.txt', self.__returnAllFilepaths())), key = str.lower)
    ...
    def printSummary(self):
        ...
        for i, filepath in enumerate(self.filepaths):
            print(row_format.format(str(i), filepath))
    ...
    def openResultsSummaryFiles(self, *args):
        ...
        for file_number in args:
            try:
                self.files.append(ResultsSummaryFile(self.filepaths[file_number]))
            except IndexError:
                pass
            except TypeError:
                pass
    ...
    def closeResultsSummaryFiles(self, *args):
        ...
        for file_number in args:
            try:
                ...
                pass
'''
file = ResultsSummaryFile('/home/matthew/Downloads/ABC130_R0H0_AUW_20180716.txt')
file.getTests()
file.printTests()
'''
